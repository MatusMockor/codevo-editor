...
#[cfg(unix)]
#[derive(Clone, Debug)]
struct DescriptorCwd(Arc<File>);

#[cfg(unix)]
impl PartialEq for DescriptorCwd {
...
impl CommandBuilder {
    ...
    /// Bind the child working directory to an already-open directory.
    ///
    /// On Unix the child enters this directory with `fchdir` immediately
    /// before exec. This prevents a pathname rename-and-replace race between
    /// validation and spawn. Call [`cwd`](Self::cwd) first if its pathname
    /// label is also needed; calling `cwd` afterwards deliberately clears the
    /// descriptor.
    #[cfg(unix)]
    pub fn cwd_fd(&mut self, directory: File) {
        self.cwd_fd = Some(DescriptorCwd(Arc::new(directory)));
    }
    ...
    #[cfg(unix)]
    pub(crate) fn get_cwd_fd(&self) -> Option<Arc<File>> {
        self.cwd_fd
            .as_ref()
            .map(|directory| Arc::clone(&directory.0))
    }
...
impl CommandBuilder {
    ...
    pub(crate) fn as_command(&self) -> anyhow::Result<std::process::Command> {
        ...
        let descriptor_bound = self.cwd_fd.is_some();
        let dir: &OsStr = self
            .cwd
            .as_ref()
            .map(|dir| dir.as_os_str())
            .filter(|dir| descriptor_bound || std::path::Path::new(dir).is_dir())
        ...
        let mut cmd = if self.is_default_prog() {
            ...
            // Defer relative executable and PATH resolution until after the
            // descriptor-bound fchdir. Resolving against `dir` in the parent
            // would let a renamed path select a program from its replacement.
            let resolved = if descriptor_bound {
                self.args[0].clone()
            } else {
                self.search_path(&self.args[0], dir)?
            };
        ...
        // A descriptor-bound cwd is installed with fchdir in the Unix
        // pre-exec hook. Reopening `dir` here would reintroduce a
        // rename-and-replace race and could fail after the original directory
        // was moved even though its descriptor remains valid.
        if self.cwd_fd.is_none() {
            cmd.current_dir(dir);
        }
        if let Some(directory) = self.get_cwd_fd() {
            unsafe {
                cmd.pre_exec(move || {
