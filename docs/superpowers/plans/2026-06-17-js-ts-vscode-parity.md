...
- JS/TS Monaco language-server providers are registered for React language ids too (`javascriptreact` and `typescriptreact`), and their active-document guard accepts those ids. TSX/JSX files therefore get the same hover, completion, navigation, code action, formatting, semantic token, inlay hint, and linked editing paths as plain JS/TS files.
