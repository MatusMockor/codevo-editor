...
### Slice: Workspace Detection Active Workspace Guard - 2026-06-21

#### Checkpoint

- Branch: `main...origin/main`
- Latest pushed commit observed:
  - `ea868ec6 Record workspace trust guard commit`
- Stash snapshot still present:
  - `stash@{Tue Jun 16 15:29:26 2026}: On main: wip macOS release CI`
- Worktree was clean at slice start.
...
- Prevent delayed workspace detection failures from a previous project tab from surfacing in the active workspace.

#### Implementation Choice

- Keep the existing active-root success guard for workspace detection and replace the catch-side global report with `reportErrorForActiveWorkspaceRoot`.
- Extend the controller test harness so tests can inject a custom workspace detection gateway.
- Add a regression where `/workspace-a` has delayed workspace detection, the user switches to `/workspace-b`, and the stale `/workspace-a` rejection cannot create a Workspace Detection notice in `/workspace-b`.

#### Acceptance Criteria

- Stale workspace detection failures do not surface after switching project tabs.
- Existing workspace trust guard and tab-switch persistence behavior still pass.
- Full controller preview tests, `npm run check`, and `git diff --check` pass.

#### Verification

- PASS: `npm test -- src/application/useWorkbenchController.preview.test.tsx -t "ignores stale workspace detection errors after switching project tabs|ignores stale workspace trust errors after switching project tabs|switches between persisted project tabs without stopping another project runtime"`
- PASS: `npm test -- src/application/useWorkbenchController.preview.test.tsx`
- PASS: `npm run check`
- PASS: `git diff --check`
...
### Slice: Directory Load Active Workspace Guard - 2026-06-21

#### Checkpoint

- Branch: `main...origin/main`
- Latest pushed commit observed:
  - `9fea7410 Record workspace settings save guard commit`
- Stash snapshot still present:
  - `stash@{Tue Jun 16 15:29:26 2026}: On main: wip macOS release CI`
- Worktree was clean at slice start.
...
- Prevent delayed directory load results or failures from a previous project tab from updating the active workspace.

#### Implementation Choice

- Add `workspacePathBelongsToRoot` using normalized root keys and separator-boundary checks.
- Gate `loadDirectory` success and error side effects on the requested directory still belonging to the active workspace root.
- Keep loading cleanup keyed by directory path so stale in-flight reads can still clear their own loading marker.
- Add a regression where `/workspace-a` directory loading remains pending, the user switches to `/workspace-b`, and the stale `/workspace-a` rejection cannot create a Workspace notice in `/workspace-b`.

#### Acceptance Criteria

- Stale directory load failures do not surface after switching project tabs.
- Stale directory load successes do not write entries into the active workspace state.
- Existing workspace detection and tab-switch persistence behavior still pass.
- Full controller preview tests, `npm run check`, and `git diff --check` pass.

#### Verification

- PASS: `npm test -- src/application/useWorkbenchController.preview.test.tsx -t "ignores stale directory load errors after switching project tabs|ignores stale workspace detection errors after switching project tabs|switches between persisted project tabs without stopping another project runtime"`
- PASS: `npm test -- src/application/useWorkbenchController.preview.test.tsx`
- PASS: `npm run check`
- PASS: `git diff --check`
...
### Slice: File Delete Active Workspace Guard - 2026-06-21

#### Checkpoint

- Branch: `main...origin/main`
- Latest pushed commit observed:
  - `514ae83c Record file rename guard commit`
- Stash snapshot still present:
  - `stash@{Tue Jun 16 15:29:26 2026}: On main: wip macOS release CI`
- Worktree was clean at slice start.
...
- Prevent delayed file delete failures or success tails from a previous project tab from surfacing in the active workspace.

#### Implementation Choice

- Capture `requestedRoot` in `deleteActiveDocument`.
- Re-check the active workspace root after filesystem delete, after JS/TS sync/watch notifications, and after directory refresh before mutating active UI state.
- Replace the catch-side global Delete File report with `reportErrorForActiveWorkspaceRoot`.
- Add a regression where `/workspace-a` starts deleting a file, the user switches to `/workspace-b`, and the stale `/workspace-a` rejection cannot create a Delete File notice in `/workspace-b`.

#### Acceptance Criteria

- Stale delete failures do not surface after switching project tabs.
- Stale delete success tails do not close or message the active workspace after a tab switch.
- Existing JS/TS delete notification ordering and rename stale guards still pass.
- Full controller preview tests, `npm run check`, and `git diff --check` pass.

#### Verification

- PASS: `npm test -- src/application/useWorkbenchController.preview.test.tsx -t "ignores stale delete errors after switching project tabs|closes a JS TS document before notifying the service that its file was deleted|ignores stale rename errors after switching project tabs"`
- PASS: `npm test -- src/application/useWorkbenchController.preview.test.tsx`
- PASS: `npm run check`
- PASS: `git diff --check`
