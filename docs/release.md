...
matches the version in `package.json`. It preserves version tags. After publishing a
beta, it creates or moves the `beta` channel tag to the release commit.
...
publish attempt removes only its owned unpublished version draft and leaves the
version tag intact. A rerun after successful publication preserves the existing
version assets, validates and reuses their `latest.json`, and can finish updating the
beta channel.
...
GitHub's `/releases/latest` endpoint excludes prereleases. The application instead
uses `/releases/download/beta/latest.json` to discover beta updates. After publishing
