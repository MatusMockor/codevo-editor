...
import { RemoteInstructionSettings } from "./RemoteInstructionSettings";
...
it("loads only expanded settings and allows choosing and clearing a stale local project", async () => {
  ...
  const render = (projects = [local]) =>
    act(() =>
      root.render(
        <RemoteInstructionSettings gateway={api} serverId="server" connected projects={projects} />,
      ),
    );
  render();
  ...
  expect(readRemoteInstructionRoot("server", "runner", "project")).toBe("/local");
  render([]);
  ...
  expect(readRemoteInstructionRoot("server", "runner", "project")).toBeUndefined();
});
it("ignores late inventory from the previous server and never exposes its mapping", async () => {
  ...
  saveRemoteInstructionRoot("first", "runner", "project", "/local");
...
it("keeps A → B → A inventory generations separate", async () => {
  ...
  expect(readRemoteInstructionRoot("a", "new-a", "project")).toBe("/local");
  expect(readRemoteInstructionRoot("a", "old-a", "project")).toBeUndefined();
});
