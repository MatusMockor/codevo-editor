import { useAgentRemoteClonePorts } from "./useAgentRemoteClonePorts";
import {
...
export interface AgentProjectCreationOptions extends AgentAddProjectOptions {
  readonly selectedServerId: string | null;
  refreshProjects(): Promise<void>;
}

export function useAgentProjectCreationLane(options: AgentProjectCreationOptions) {
  ...
  const [serverId, setServerId] = useState(() =>
    ...
    (saved.pending.environment === null
      ...
      : saved.remoteGateway === remoteGateway)
      ? saved.serverId
      : savedRemote?.runnerGateway === remoteGateway
        ? savedRemote.serverId
        : options.selectedServerId,
  );
  const { lookupGateway, directoryGateway } = useAgentRemoteClonePorts(
    remoteGateway,
    serverId,
    session,
  );
  ...
  const [visible, setVisible] = useState(restored?.activationReceipt != null);
  const [launch, setLaunch] = useState<AgentLaunchOptions | null>(restored?.launch ?? null);
  const [isolation, setIsolation] = useState<AgentTaskIsolation>(restored?.isolation ?? "in-place");
  const [completedProject, setCompletedProject] = useState<AgentProjectDescriptor | null>(
    restored?.completedProject ?? null,
  );
  const activationReceipt = useRef(restored?.activationReceipt ?? null);
  ...
  const retryDraftKey = useRef<string | null>(null);
  ...
  const started = (id: string, name: string, environment: string | null, select = true) => {
    ...
    if (retryDraftKey.current === null) {
      setLaunch(null);
      setIsolation("in-place");
    }
    activationReceipt.current = null;
    ...
    setPending({
      ...
      environment,
      target: null,
      draftKey: retryDraftKey.current ?? `clone:${environment ?? "local"}:${id}`,
    });
    retryDraftKey.current = null;
  ...
  const addProject = useAgentAddProject({
    ...options,
    onProjectAddFailed(failure) {
      const receipt = activationReceipt.current;
      if (receipt === null || receipt.id !== pendingRef.current?.id) return;
      activationReceipt.current = null;
      setActivationFailed(true);
      setError(
        failure instanceof Error
          ? failure.message.slice(0, 500)
          : "Could not open the cloned project. Retry opening it.",
      );
      if (session?.current?.activationReceipt === receipt)
        session.current = { ...session.current, activationReceipt: null };
    },
    onProjectAdded(project) {
      const activation = activationReceipt.current;
      if (activation !== null) {
        if (
          activation.id !== pendingRef.current?.id ||
          activation.path !== project.rootPath ||
          normalizedWorkspaceRootKey(activation.path) !== project.rootKey
        )
          return;
        activationReceipt.current = null;
    ...
    session.current =
      pending !== null && ownerCurrent && openReceipt.current === null
        ? {
            ...
            isolation,
            completedProject,
            activationReceipt: activationReceipt.current,
            serverId,
            workspaceOwner: cloneWorkspaceOwner,
          }
        : null;
  ...
  const liveCompletedProject =
    remoteTarget !== null
      ? (options.projects.find((project) => project.rootKey === remoteTarget) ?? null)
      : completedProject === null
        ? null
        : (options.projects.find(
            (project) =>
              project.rootKey === completedProject.rootKey &&
              project.ownerId === completedProject.ownerId &&
              project.generation === completedProject.generation,
          ) ?? null);
  return {
    addProject,
    local,
    ...
    changeIsolation: setIsolation,
    completedProject: liveCompletedProject,
    activateCompleted() {
      const current = pendingRef.current;
      if (
        current?.target?.kind !== "local" ||
        !ownerCurrent ||
        !visible ||
        completedProject !== null ||
        activationFailed ||
      ...
      if (
        ...
        return;
      activationReceipt.current = { id: current.id, path: current.target.path };
    ...
    retry() {
      ...
      retryDraftKey.current =
        pending?.draftKey ??
        (pending ? `clone:${pending.environment ?? "local"}:${pending.id}` : null);
      retryDraft.current =
    ...
    dismiss() {
      if (local.pending || pendingClone?.status === "running" || pendingClone?.status === "queued")
        return;
