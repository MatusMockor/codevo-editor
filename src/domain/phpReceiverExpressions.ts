...
export const PHP_NEW_RECEIVER_PATTERN = String.raw`(?:\(\s*new\s+${PHP_CLASS_NAME_PATTERN}\s*(?:\([^)]*\))?\s*\)|new\s+${PHP_CLASS_NAME_PATTERN}\s*(?:\([^)]*\))?)`;
export const PHP_STATIC_CALL_RECEIVER_PATTERN = String.raw`${PHP_CLASS_NAME_PATTERN}\s*::\s*[A-Za-z_][A-Za-z0-9_]*\s*\([^)]*\)`;
export const PHP_MEMBER_RECEIVER_PATTERN = String.raw`\$[A-Za-z_][A-Za-z0-9_]*`;
export const PHP_EXPRESSION_RECEIVER_PATTERN = String.raw`(?:${PHP_NEW_RECEIVER_PATTERN}|${PHP_MEMBER_RECEIVER_PATTERN}|${PHP_STATIC_CALL_RECEIVER_PATTERN}|${PHP_FUNCTION_CALL_RECEIVER_PATTERN})`;
