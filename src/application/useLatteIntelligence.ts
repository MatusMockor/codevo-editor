...
function netteCreateComponentViewDataEntryFromSource(
  deps: LatteIntelligenceDependencies,
  source: string,
...
function componentViewOwnerNameFromType(typeName: string | null): string | null {
...
function phpTypeNamesEqual(left: string, right: string): boolean {
 ...
 * template tree instead of next to `SomethingControl.php`. PhpStorm still gives
 * that template the control's view-data because it follows the
 * `createComponentSomething(): SomethingControl` factory. This bridge keeps the
 * same behavior conservative: it only considers factories whose component name
 * is named by the active template path, and it only adds owners whose return
 * type looks like a `*Control` class.
 */
async function factoryDerivedLatteCandidateViewNames(
  context: LatteExpressionResolutionContext,
  templateRelativePath: string,
): Promise<string[]> {
  const { deps, isRequestedRootActive, requestedRoot } = context;
