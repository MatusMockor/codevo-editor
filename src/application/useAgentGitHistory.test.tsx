...
it("bounds retained commits and files and navigates numeric cursor pages", async () => {
  ...
  expect(gateway.getCommitLog).toHaveBeenLastCalledWith("/a", { limit: 51, cursor: "50" });
  expect(result.details).toBeNull();
  await act(async () => {
    result.previousPage();
  });
