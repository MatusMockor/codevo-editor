...
type HookOptions = UsePhpFrameworkMorphMapResolverOptions;
...
function renderHook(options: HookOptions) {
  ...
  function Harness({ hookOptions }: { hookOptions: HookOptions }) {
    captured.api = usePhpFrameworkMorphMapResolver(hookOptions);
    return null;
  }
...
describe("usePhpFrameworkMorphMapResolver", () => {
  ...
  it("resets cached morph map model type results on request", async () => {
    const options = makeOptions();
    const harness = renderHook(options);
    ...
    harness.api().resetPhpFrameworkMorphMapModelTypeCache();
