...
export interface WorkbenchEditorGroupCloseLifecycleDependencies {
  ...
  setMarkdownPreviewTabs: Dispatch<SetStateAction<Record<string, MarkdownPreviewTab>>>;
  setEslintDiagnosticsByRoot: Dispatch<SetStateAction<EslintDiagnosticsByRoot>>;
  setPhpstanDiagnosticsByRoot: Dispatch<SetStateAction<PhpstanDiagnosticsByRoot>>;
...
export interface WorkbenchEditorGroupCloseLifecycle {
  closeDocument: (path: string, options?: DocumentCloseOptions) => Promise<CloseCompletion>;
...
export function useWorkbenchEditorGroupCloseLifecycle(
  ...
  const clearCodeQualityDiagnostics = useCallback(
    ...
    [currentWorkspaceRootRef, setEslintDiagnosticsByRoot, setPhpstanDiagnosticsByRoot],
  );

  const closeVisualDocumentImmediately = useCallback(
    (path: string, clearDiagnostics = true) => {
      if (clearDiagnostics) {
      ...
      updateEditorGroups((latest) => ({
        ...
        groups: {
          ...latest.groups,
          [latest.activeGroupId]: closeEditorGroupPath(latest.groups[latest.activeGroupId], path),
        },
      }));
    },
    [
  ...
  const commitMembershipClose = useCallback(
    (
      ...
      closeFinalMembershipImmediately(groupId, path, options, clearDiagnostics);
    },
  ...
  const targetState = useCallback(
    (target: EditorCloseTarget, expectedDocument: EditorDocument = target.identity.document) => {
  ...
  const externalAliasesAreCurrent = useCallback(
    (scope: CapturedCloseScope): boolean => {
  ...
  const conditionalCommit = useCallback(
    (
      ...
      for (const [path, capturedDocument] of scope.documents) {
        ...
        const target = scope.targets.find((candidate) => candidate.identity.path === path);
        if (
          requireClean &&
          isDirty(live) &&
          (!target || !acknowledgedDocumentIdentities.has(target.identity.documentIdentity))
        ) {
          return staleCommit(targets, "target-replaced");
        }
      }
  ...
  const runWithTargetWriteDrains = useCallback(
    async <T>(targets: readonly EditorCloseTarget[], operation: () => Promise<T>): Promise<T> => {
  ...
  const executeCapturedClose = useCallback(
    async (
      scope: CapturedCloseScope,
      ...
      try {
        decision = await dirtyCloseDecisionPort.decideDirtyClose({
          ...
          documentNames: scope.targets.map((target) => target.identity.document.name),
        });
      } catch {
        return "blocked";
      }
      ...
      const transactionTargets = [...targetGroups.values()]
        .map(([target]) => target)
      ...
      const transaction = new DirtyCloseSaveTransaction<EditorCloseIdentity>({
        saveTarget: async (target) => {
          ...
          if (saveResult.status === "saved") {
            if (saveResult.contentIsCurrent) {
              ...
              acknowledgedDocumentIdentities.add(target.identity.documentIdentity);
            }
          }
          return saveResult;
        },
        ...
        revalidateTarget: (target) =>
          targetState(target, savedDocuments.get(target.identity.path) ?? target.identity.document),
        commitCloseConditionally: (targets) =>
          conditionalCommit(scope, targets, true, savedDocuments, acknowledgedDocumentIdentities),
      });
      let result;
      try {
  ...
  const captureTargets = useCallback(
    (paths: readonly string[]): CapturedCloseTargets | null => {
      const rootPath = currentWorkspaceRootRef.current;
      const decisionPaths = paths.filter((path) => {
        const document = documentsRef.current[path];
        return Boolean(document) && (isDirty(document) || hasExternalFileConflict(rootPath, path));
      });
      ...
      for (const path of decisionPaths) {
        ...
        const existingSnapshot = snapshotsByDocumentIdentity.get(documentIdentity);
        if (existingSnapshot && !sameCanonicalSnapshot(existingSnapshot, document)) {
          return null;
        }
      ...
      for (const path of editorGroupsUniquePaths(editorGroupsRef.current)) {
        ...
        const decisionSnapshot = snapshotsByDocumentIdentity.get(documentIdentity);
        if (decisionSnapshot && !sameCanonicalSnapshot(decisionSnapshot, document)) {
          return null;
        }
        if (decisionSnapshot) {
  ...
  const closeDocument = useCallback(
    (path: string, options: DocumentCloseOptions = {}) =>
      closeDocumentInEditorGroup(editorGroupsRef.current.activeGroupId, path, options),
  ...
  const closeActiveEditorGroup = useCallback(async (): Promise<CloseCompletion> => {
    ...
    const commit = () =>
      commitGroupCloseAtomically(editorGroups, groupId, editorGroupVisiblePaths(group), () => {
        for (const path of editorGroupVisiblePaths(group)) {
          commitMembershipClose(groupId, path, { skipConfirmation: true }, false);
        }
    ...
    const completion = await executeCapturedClose(
      {
        documents: captureDocuments(documentsRef.current, closeResult.finalMembershipPaths),
        editorGroups,
        externalAliases: capture.externalAliases,
        targets: capture.targets,
        commit,
  ...
  }, [closeActiveEditorGroup, closeActiveSurface, closeDocumentInEditorGroup, editorGroupsRef]);
...
function sameCapturedDocument(live: EditorDocument, captured: EditorDocument): boolean {
  return (
...
function sameEditorCloseMembership(live: EditorGroupsState, captured: EditorGroupsState): boolean {
  ...
  return capturedGroupIds.every((groupId) => {
    ...
    return (
      Boolean(liveGroup) &&
      Boolean(capturedGroup) &&
      liveGroup.activePath === capturedGroup.activePath &&
      samePaths(editorGroupVisiblePaths(liveGroup), editorGroupVisiblePaths(capturedGroup))
    );
  });
}

function sameCanonicalSnapshot(first: EditorDocument, second: EditorDocument): boolean {
  return (
