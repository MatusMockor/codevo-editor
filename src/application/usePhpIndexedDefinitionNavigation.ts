...
async function invokeNavigationHandler<Argument>(
  ...
  isNavigationFinalizable: () => boolean,
): Promise<boolean> {
  const handled = request ? await handler(argument, request) : await handler(argument);

  if (handled ? !isNavigationFinalizable() : !isNavigationActive()) {
    return false;
  }
...
export function usePhpIndexedDefinitionNavigation({
  ...
  const goToIndexedSymbolDefinition = useCallback(
    async (request?: NavigationRequest): Promise<boolean> => {
      ...
      const isNavigationFinalizable = () =>
        isRequestedRootActive() && canFinalizeNavigation(request);
      ...
      const symbolName = identifierAtEditorPosition(activeDocument.content, editorPosition);
      ...
      const openIndexedSymbolByName = async (query: string): Promise<boolean> => {
        ...
        const symbols = await projectSymbolSearch.searchProjectSymbols(requestedRoot, query, 25);

        if (!isNavigationActive()) {
          return false;
        }

        const target = bestIndexedSymbolMatch(symbols, query, activeDocument.path);
        ...
        if (!isRequestedRootActive() || !canFinalizeNavigation(request)) {
          return false;
        }
      ...
      try {
        if (activeDocument.language !== "php") {
          ...
          if (openedIndexedTarget ? !isNavigationFinalizable() : !isNavigationActive()) {
            return false;
          }
        ...
        if (openedFrameworkTarget ? !isNavigationFinalizable() : !isNavigationActive()) {
          return false;
        }
        ...
        if (openedClassTarget ? !isNavigationFinalizable() : !isNavigationActive()) {
          return false;
        }
        ...
        reportErrorForActiveWorkspaceRoot(requestedRoot, "Go to Definition", error);
        return false;
      }
    },
    [
